# Compute MSE across genes of same module
...
for(i in 1:length(files)){
  ...
  mse <- matrix(nrow = length(unique(membership)), ncol = ncol(predicted),
                dimnames = list(unique(membership), colnames(predicted)))
  for(module in unique(membership)){
    
    genes <- names(which(membership == module))
    genes <- intersect(genes, rownames(predicted))
    
    # compute MSE for each sample across module genes
    mse[module,] <- sapply(1:ncol(predicted),
                           function(j) ComputeMSE(predicted[genes,j],
                                                  centered_data[genes,j]))
  }
  
  saveRDS(mse, gsub("net_predictions",paste0(METHOD,"modules_mse"),files[i]))
}
