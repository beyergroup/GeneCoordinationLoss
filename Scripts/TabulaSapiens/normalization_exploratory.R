...
# log CPM
sizeFactors(sce_10X) <- colSums(assay(sce_10X, "decontXcounts"))/(10^6)
sce_10X <- logNormCounts(sce_10X, size.factors = sizeFactors(sce_10X),
                         log = T, pseudo.count = 1, center.size.factors = F,
                         assay.type = "decontXcounts")
sizeFactors(sce_SS2) <- colSums(assay(sce_SS2, "decontXcounts"))/(10^6)
sce_SS2 <- logNormCounts(sce_SS2, size.factors = sizeFactors(sce_SS2),
                         log = T, pseudo.count = 1, center.size.factors = F,
                         assay.type = "decontXcounts")
...
# pdf(paste0("../../Plots/Tabula_Sapiens/QC/",TISSUE,"_norm_genequant.pdf"),
#     height = 12, width = 10)
...
for(cell_type in cell_types){
  ...
  plot.data <- rbind.data.frame(plot.data,
                                data.frame("Cell1" = as.matrix(assay(tmp_10X,"logcounts"))[,cells[1]],
                                           "Cell2" = as.matrix(assay(tmp_10X,"logcounts"))[,cells[2]],
                                           "CellType" = cell_type,
                                           "Method" = "10X"))
  ...
  plot.data <- rbind.data.frame(plot.data,
                                data.frame("Cell1" = as.matrix(assay(tmp_SS2,"logcounts"))[,cells[1]],
                                           "Cell2" = as.matrix(assay(tmp_SS2,"logcounts"))[,cells[2]],
                                           "CellType" = cell_type,
                                           "Method" = "Smart-seq2"))
  
  # expression quantiles (5 quantiles)
  means <- rowMeans(as.matrix(assay(tmp_10X,"logcounts")))
  ...
  for(qq in 1:5){
    ...
    genes <- names(which(q == qq))
    cor <- apply(log2(1+as.matrix(assay(tmp_10X,"logcounts")))[genes,], 1,
    ...
    # fit <- apply(as.matrix(assay(tmp_10X))[genes,], 1,
  ...
  means <- rowMeans(as.matrix(assay(tmp_SS2,"logcounts")))
  ...
  for(qq in 1:5){
    genes <- names(which(q == qq))
    cor <- apply(log2(1+as.matrix(assay(tmp_SS2,"logcounts")))[genes,], 1,
    ...
    rm(genes,cor); gc()
    # # fit kernel smooth function per gene (expression ~ log10 lib size)
    # fit <- apply(as.matrix(assay(tmp_SS2))[genes,], 1,
  ...
  #   ylab("Log CPM") + guides(fill = FALSE) +
...
png(paste0("../../Plots/Tabula_Sapiens/QC/",TISSUE,"_norm_cellvscell.png"),
    width = 900,
    # width = 600,
    height = 700)
    # height = 1000)
...
pdf(paste0("../../Plots/Tabula_Sapiens/QC/",TISSUE,"_norm_corr.pdf"),
    # width = 13, # Lung
    # width = 6.5, # Pancreas
    width = 10, # Vasculature
    height = 15)
ggarrange(plotlist = plots, nrow = 2)
dev.off()
