...
tissue_labels_dash_all <- c(tissue_labels_dash_all, "AllCells" = "All celltypes",
...
for(i in 1:length(files)){
  ...
  plots[[t]] <- ggplot(plot.data) + 
    ...
    scale_size_manual(values = c("Original" = 2, "Random" = 1)) +
...
pdf(paste0(PLOT_DIR,"/performance_against_crosstissue_",MODE,".pdf"), height = 5, width = 9)
ggarrange(plotlist = plots[order(names(plots))], ncol = 3, nrow = 2,
