...
NET = args[1]
EXPR_DIR = "Outputs/5_Predictability"  # args[2]
OUTDIR = "Outputs/5_Predictability" # args[3]
...
for(mode in c("cis","trans")){
  ...
  net <- as.matrix(ReadRDS(paste0("Outputs/5_Predictability/",mode,"_net_chrarm.rds")))
  ...
  for(file in data_files){
    ...
    saveRDS(pred, paste0(OUTDIR,"/",
                         gsub("_data",paste0("_",mode,"_net_predictions"),file)))
