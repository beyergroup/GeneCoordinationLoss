...
#                     full.names = T)
files <- list.files("Outputs/5_Predictability",
                    pattern = "_sampled_centered_correlations_spearman.rds",
                    full.names = T)
...
for(i in 1:length(old_files)){
  ...
  # lfcs <- ComputeCorrelationLFC(condition = old[genes], control = young[genes])
  
  saveRDS(lfcs, paste0("Outputs/5_Predictability/",tissue,"_corLFCs_YvsO_adj_spearman.rds"))
  # saveRDS(lfcs, paste0("Outputs/5_Predictability/",tissue,"_corLFCs_YvsO_adj.rds"))
  # saveRDS(lfcs, paste0("Outputs/5_Predictability/",tissue,"_corLFCs_YvsO.rds"))
}
